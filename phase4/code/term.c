...
//  fprintf (infF, "before printing\n");
  //fterm = PCB[pid]->clientF;
 if (pid != -1) {

  fprintf (PCB[pid]->clientF, "%s\n", outstr);
//  fprintf (infF, "after printing\n");
 // fprintf (, "%s\n", outstr);
  fflush (PCB[pid]->clientF);
 // fprintf (infF, "after printing\n");
...
{ int ret;
  ...
  sem_init(&termIOmutex, 0, 1);
  sem_init(&empty, 0, 0);
  ret = pthread_create(&termThread, NULL, termIO , (void *) 0);
  if (ret){
  	fprintf(stdout, "ERROR; return code from pthread_create() is %d\n", ret);
  } else {
  	fprintf(stdout, "TermIO thread has been created successfully\n");
  }
}

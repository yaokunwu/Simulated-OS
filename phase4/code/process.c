...
{ PCB[pid]->exeStatus = CPU.exeStatus;
  ...
  sleep(1);  // ADDED MYSELF, because at the end of process finishing, the termIO thread need to access the pid, so should after free the pid
  numUserProcess--;
  clean_process (pid); 
...
int submit_process (char *fname, FILE *socketF)
{ int pid, ret, i;
  ...
  { pid = new_PCB ();
	...
	PCB[pid]->clientF = socketF;
//	PCB[pid]->socketno = socketno;
...
{ int pid, intime=0;
  ...
  { // *** ADD CODE to execute the ready process (with pid# = pid)
    ...
    context_in(pid);
    event = add_timer (cpuQuantum, pid, actTQinterrupt, oneTimeTimer);
